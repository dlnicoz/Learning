...
> Primitive data types (like numbers, strings, and booleans) are immutable, meaning their values cannot be changed.
> Variables (let or var) that reference those values can be reassigned to new values, which is why age = 34 works.
> Use const if you want to prevent reassignment of a variable.
...
>  Primitive types are not objects but can behave like objects temporarily due to wrapper objects.
>  Functions, arrays, and other complex types are objects.
>  The prototype system in JavaScript makes it feel like "everything is an object," but that's not strictly true.
